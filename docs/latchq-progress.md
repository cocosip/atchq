...
- 2026-09-23:**五轮走查(毒消息语义/互斥/配置校验/零警告)修复落地**。① P1 批次中间毒消息丢前缀:`read` 批次内毒消息抛异常时,同批已成功反序列化的前缀条目随异常整体丢弃(已消费未提交、本会话不可恢复),且 [truncate, poison.index) 成为无 commit 覆盖的空洞,合并停滞至强制跳过把前缀一并放弃并随 checkpoint 持久化(重启不再重投);`forceCompleteGapTimeoutMillis=0` 时永久停摆。`LatchQDeserializationException` 新增 `getSuccessfullyRead()` 自类型化泛型访问器(Java 禁止泛型类继承 Throwable,JLS 8.1.2,故异常保持非泛型),`read` 两条路径把已读前缀传入异常;调用方先处理并 commit 前缀再 `forceCommitGap` 毒条目,区间衔接后合并即刻推进;README/两个示例/javadoc 同步,新增 mid-batch 与首条中毒 2 个测试;② P2 后台循环 0 间隔热循环:`complete/checkpoint/cleanupIntervalMillis` 校验收紧为正数(`checkpoint=0` 繁忙时等于持续 fsync;`syncIntervalMillis=0` 仍表示禁用),README 配置表同步;③ P2 export/cleanup 互斥的检查-递增竞态窗口(cleanup 通过计数检查后 export 才注册自己,可删掉 export 正在扫描的历史 cycle):`activeExports` 计数器改为 `ReentrantReadWriteLock`,export 全程持读锁、cleanup try-lock 写锁跳过本轮,互斥严密;④ P3 一批:`metrics()` 改走无生命周期检查的 `lastIndexInternal()`(消除与 close 竞态抛异常,与"任意生命周期可调"承诺一致);`LatchQueueBuilder.build()` 去掉统一包装(保留 LatchQInvalidConfigurationException 具体类型,与工厂路径一致);`CheckpointStore.write` 串行化(close join 超时后与 checkpoint 线程并发写同一 checkpoint.tmp 的理论窗口);`initialize()` 失败路径关闭已构建的 queue(防句柄/后台线程泄漏);`TypeStorageNaming.sanitize` 处理 Windows 保留设备名与结尾点/空格;`maxMessageSizeBytes` 校验上限 Integer.MAX_VALUE(消除 blockSize 推导的理论溢出);spring 示例 stop() 补 awaitTermination;⑤ 构建:compiler 插件开启 `-Xlint:all,-processing -Werror`(-processing 为 Spring 注解无编译期处理器的固有噪声),补 serialVersionUID、Jackson 2.20 弃用 `fields()` 改 `properties()`,全仓编译零警告。全量 verify 通过(83 core + 7 starter)。
